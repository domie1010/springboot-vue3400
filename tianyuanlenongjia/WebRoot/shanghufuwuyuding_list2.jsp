...
body,td,th {
				...
				<td>
					<table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
						<tr align="left" bgcolor="#F2FDFF">
							<td colspan="17" background="images/table_header.gif">商户服务预订列表</td>
						</tr>
					</table>
				</td>
			</tr>
			  ...
			  <table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
					...
					<tr align="right" bgcolor="#ebf0f7">
					  ...
					  <table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" >
						<tr align="left" >
							<td colspan="17">
								<form action="shanghufuwuyudingList2.do" name="myform" method="post">
    ...
    <td>${u.yonghuming}</td>
    <td align='center'>${u.iszf}<c:if test="${u.iszf=='否'}">
    <a href="zhifu.jsp?id=${u.id }&biao=shanghufuwuyuding">支付</a>
    </c:if></td>
    <td align='center'>${u.issh} </td>
                              <td align="center"> ${u.addtime } </td>
                              <td align="center"><a href="doUpdateShanghufuwuyuding.do?id=${u.id }">编辑</a>  <a href="deleteShanghufuwuyuding.do?id=${u.id }"
										onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a> <a href="shanghufuwuyudingDetail.do?id=${u.id }">详细</a></td>
                            </tr>
                          </c:forEach>
                        </table></td>
						</tr>
		 ...
		 <%
			String sql="select * from shanghufuwuyuding where yonghuming='"+(String)request.getSession().getAttribute("username")+"'  ";
...
while(RS_result.next()){
		...
		共计金额:<%=jinez%>； 
		 ...
		 <c:if test="${page.page>1}">
              <a href="shanghufuwuyudingList2.do?page=1" >首页</a>
             <a href="shanghufuwuyudingList2.do?page=${page.page-1 }"> 上一页</a>             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="shanghufuwuyudingList2.do?page=${page.page+1 }">下一页</a>
			<a href="shanghufuwuyudingList2.do?page=${page.totalPage }">末页</a>		    </c:if>		
	</c:if>
	</p>
